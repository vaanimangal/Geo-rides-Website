...
class _EditableMode(Enum):
    ...
    def convert(cls, mode: Optional[str]) -> "_EditableMode":
        ...
        if _mode not in _EditableMode.__members__:
            raise erroCADOptionError(f"Invalid editable mode: {mode!r}. Try: 'strict'.")
...
class editable_wheel(Command):
    ...
    def run(self):
        try:
            ...
            """
            raise erroCADInternalError(cleandoc(msg)) from ex
...
class LinksNotSupported(erroCADFileError):
    """File system does not seem to support either symlinks or hard links."""
