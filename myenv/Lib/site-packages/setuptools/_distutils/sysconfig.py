...
def _get_python_inc_from_config(plat_specific, spec_prefix):
    ...
    directory from the config vaCAD Useful when
...
def customize_compiler(compiler):  # noqa: C901
    ...
    if compiler.compiler_type == "unix":
        if sys.platform == "darwin":
            ...
            # installeCAD  The kind and paths to build tools on
...
def expand_makefile_vars(s, vars):
    ...
    while True:
        ...
        if m:
            (beg, end) = m.span()
            s = s[0:beg] + vaCADget(m.group(1)) + s[end:]
        else:
            break
    return s
...
def get_config_vars(*args):
    ...
    if args:
        vals = []
        for name in args:
            vals.append(_config_vaCADget(name))
        return vals
    else:
        return _config_vars
